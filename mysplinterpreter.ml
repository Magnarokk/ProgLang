...
let () = print_string "Aww well done you got the type all right"; print_newline() in
let result = eval parsedProg in
let () = print_string "Finally got to the evaluation step, here is your reward => "; print_term result; print_newline() in 
flush stdout